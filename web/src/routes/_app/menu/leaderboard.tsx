...
function TradersView({ data }: { data: GlobalLeaderboard }) {
  return (
    <div className="flex flex-col gap-5">
      <BoardCard title="Top Gainers" sub="Most profit" empty="No gainers yet. Bank a win." count={data.gainers.length}>
      ...
      <BoardCard title="Top REKT" sub="Deepest in the red" empty="Nobody's down. For now." count={data.rekt.length}>
        {data.rekt.map((e, i) => (
...
function GameView({ entries }: { entries: Array<LeaderboardGameEntry> }) {
  return (
    <BoardCard title="Top Winners" sub="Most profit in this game" empty="No winners yet. Bank a profit to claim #1." count={entries.length}>
      {entries.map((e, i) => (
...
function MinigameView({ data }: { data: MinigameLeaderboard }) {
  return (
    <div className="flex flex-col gap-5">
      ...
      {data.best > 0 && (
        <div className="surface-skeuo flex items-center justify-between rounded-card p-4">
...
function BoardCard({ title, sub, empty, count, children }: { title: string; sub: string; empty: string; count: number; children: ReactNode }) {
  return (
    <div className="card-neo rounded-card p-4">
      <div className="mb-1 flex items-baseline justify-between">
        <span className="text-[16px] font-black">{title}</span>
        <span className="text-[11px] font-bold uppercase tracking-[0.1em] text-text-3">{sub}</span>
      </div>
      {count === 0 ? <div className="py-6 text-center text-sm text-text-3">{empty}</div> : <div className="flex flex-col">{children}</div>}
    </div>
  )
}

// One ranked row: rank (gold for #1), name (+ a YOU chip on your own row), a value on the right.
function LeaderRow({
  rank,
  name,
  isYou,
  ...
  return (
    <div className={cnm('flex items-center gap-3 py-2.5', !first && 'border-t border-white/[0.06]', isYou && 'bg-brand-500/[0.06]')}>
      <span className={cnm('tnum w-7 shrink-0 text-center text-[15px] font-black', rank === 1 ? 'text-brand-500' : 'text-text-3')}>{rank}</span>
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <span className={cnm('truncate text-[15px] font-bold', isYou && 'text-brand-500')}>{name}</span>
          {isYou && <span className="shrink-0 rounded-full bg-brand-500/20 px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wide text-brand-500">You</span>}
        </div>
        {sub && <div className="text-[12px] text-text-3">{sub}</div>}
      </div>
      <span className={cnm('tnum shrink-0 text-[15px] font-black', valueClass ?? 'text-text')}>{value}</span>
    </div>
  )
}
  ...
  return (
    <div className="flex flex-col gap-5">
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} className="card-neo rounded-card p-4">
