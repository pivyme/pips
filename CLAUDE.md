...
- Real Predict has real continuous `leverage: u64` (any `L >= 1`, `expiry_market.move:232`); plumb it through the real builder, do not clamp to 1. The root capability box line "no leverage/margin inside Predict" is stale for testnet-real; it gets its correct rewrite in Wave 002's harvest (LUCKY leverage redesign folded into that wave), not before. (L-009)
- On `testnet` (real Predict) use the tiniest amounts possible: target 0.01 DUSDC, 0.1 max, testnet-real gets its own tiny `MIN_STAKE`/default stake, and QA does the MINIMUM real plays (one mint + one cash-out + one settled), never a loop. Free-localnet spending habits do not carry over. (L-010)
