...
3. **What actually ships today is mapped in [`docs/CURRENT_PRODUCT_OVERVIEW.md`](./docs/CURRENT_PRODUCT_OVERVIEW.md)**: every route, game, drawer page, control binding, API group, and known seam. Read it before hunting through code to learn the product. `docs/FLOW.md` is the older target flow and is partly historical now; where the two disagree, the overview wins. The frontend is **not a normal dashboard**. It is a persistent console shell with a swappable screen. Read [`docs/DESIGN.md`](./docs/DESIGN.md) (how it looks) and [`docs/FLOW.md`](./docs/FLOW.md) (how it moves) before touching UI. The UI has **two distinct visual languages**: the App Surface (the menu drawer, settings, landing, modals) is iOS clean with rounded cards, per DESIGN.md; **everything inside the device screen (Home + all games) is the Teenage Engineering instrument language in [`docs/SCREEN.md`](./docs/SCREEN.md), flat black with electric high-contrast ink, no rounded cards.** Read SCREEN.md before touching any `/games/*` screen. The ADMIN dashboard at `/admin` wears the same palette and the same `@/ui/*` primitives in a dense desktop posture (no device, no WebGL); its system is [`docs/DESIGN-SYSTEM.md`](./docs/DESIGN-SYSTEM.md) Part III, alongside the app surface and the hardware kit.
...
├── docs/
│   ├── CURRENT_PRODUCT_OVERVIEW.md  What ships today: every page, feature, and seam (start here)
...
Eight trading games ship today. **Three are public** (Lucky, Range, Moonshot) and **five are lab games** (Pin, Snipe, Press, Rush, Breakout): real routes, real console, real DUSDC, real settlement, visible only to an ADMIN so they can be played and judged before anyone else sees them. `/dev/games` is the concept index, with every concept's status and a link into the playable ones.

The roster lives in **one** place, `backend/src/config/games.ts` (`GAMES`, `PUBLIC_GAMES`, `isLabGame`, `canSeeGame`), and promoting a game is a single line out of `LAB`. The gate that matters is the server's: a lab game answers **404, not 403**, byte-identical to a name that was never registered, so an unreleased game's existence is not leaked. On the web, `useLabGate()` (`web/src/lib/lab.ts`) bounces a non-admin off the route and the cartridge grid renders `LAB_GAMES` only for an ADMIN, but that is tidiness, not the gate. Adding a game touches the checklist in `bigdev/plans/cont/04-GAMES-LAB.md` §3 and nothing else; the designs live in `docs/games-ideation/CONCEPTS.md`, never in the plan.
...
> 4. Never `await track()`, never call it in a render body, never track continuous data (ticks, frames, scroll). If it fires more than once per user action it is not an event. A scrubbed control uses `trackSettled()` (debounced to the value they landed on); a mount that can re-enter uses `trackOnce()` (once per session, L-024).
...
| `bigdev/plans/cont/04-GAMES-LAB.md` | **The games lab, and five new games (PIN, SNIPE, PRESS, RUSH, BREAKOUT).** Where lab games live (real routes, ADMIN gated, 404 not 403), the `/dev/games` concept index, the add-a-game checklist across both pillars, per-leg economics + the real-play QA rule, per-game engineering notes, the BREAKOUT gate. Designs are NOT here, they live in `docs/games-ideation/`. Read for anything touching a game, the console bindings, or adding one |
| `bigdev/plans/cont/03-ADMIN-DASHBOARD.md` | **In-house admin dashboard (error tracking + product analytics + ops health).** `User.specialRoles` + ADMIN gate, DB-backed settings in `AppConfig` (zero new env vars), Sui-aware error fingerprinting/grouping, the AI brief, the `web/src/admin/` desktop dashboard (one folder, device-free but on the product's palette and primitives), the event allowlist + `track()` contract, 13 ops detectors, retention guardrails. Read for anything analytics, error tracking, roles, or `/admin` |
...
```bash
# the gate, all five, green before any commit
cd web && bunx tsc --noEmit        # fast typecheck gate (the loop's baseline check)
cd web && bun run check:admin      # admin encapsulation + shared-const drift (web lint is NOT a gate, L-020)
cd web && bun run test             # vitest; ignore the "Vite server won't exit" line, read the exit code
cd backend && bun run typecheck    # backend typecheck gate
cd backend && bun run lint         # backend lint IS a gate, keep it at 0 errors (L-017)
cd backend && bun test             # bun's runner over colocated *.test.ts
...
- An invariant is only enforced if it fails the gate that actually runs. `web`'s `bun run lint` is pre-existing red and nobody runs it, so architectural rules there (e.g. banning device-shell imports inside `src/admin/`) go in a small check script wired next to `bunx tsc --noEmit` in `web/package.json`, not in eslint. Add the eslint rule for editor squiggles if you like, never as the enforcement. If breaking a documented rule turns nothing red, it is a suggestion. (L-020)
- Never dedupe, group, or suppress on an interpolated message string: our messages carry play ids, amounts, and object ids, so every occurrence is unique and the dedupe silently never fires (this is why `alert()`'s message-text dedupe fails). Key off a stable computed value: a fingerprint (`kind + code + normalize(message) + topOwnFrame`, percent-decoding FIRST since gRPC-web errors arrive encoded, Sui `MoveAbort`s collapsing to `module::function::code`) for errors, the detector key for monitors. Fire on transition into a bad state plus once on recovery, never every tick, and keep expected aborts counted at `warn` and unpaged. (L-021)
- A non-secret tunable that is not needed before the DB is up is a SETTING, not an env var: one const carrying type + default + bounds, overrides stored as JSON in the existing `AppConfig` table, clamped on READ every time, cached ~30s, unknown keys rejected. `main-config.ts` is already ~385 lines of env knobs and each new one is a value you must remember exists, set in Dokploy, and redeploy to change. Secrets and boot-critical values (`DATABASE_URL`, `JWT_SECRET`, wallet keys, webhook URLs) stay in env forever, and anything version-shaped stamps itself at build time, never by hand. (L-022)
- A crucial path is not done until its test has been RUN, not merely written. Anything whose failure is silent (analytics, error grouping, access control, redaction, dashboard aggregates, guardrails on deletion) gets its test in the same phase as the code, executed, with the real output pasted before the box is checked. Use the harnesses that exist: `cd backend && bun test` (colocated `*.test.ts`, bun's runner), `cd web && bun run test` (vitest + jsdom + testing-library; its post-run "Vite server won't exit" line is noise, read the exit code), and a throw-on-failure `backend/scripts/<name>.ts` proof script for claims a unit test structurally cannot make (the `post-deploy-check.ts` idiom). Assert the STORED row whenever the server is meant to overwrite client input, and watch a guard fail once before trusting it. Never loop real plays in a test (L-010). (L-023)
- On the client, any event fired from a mount, an error boundary, or a retry goes through `trackOnce()` (`web/src/lib/track.ts`: per-session, keyed on name + props, bounded at 50 keys), never on the 200-item queue cap alone, which bounds MEMORY not rows. A re-mounting crash boundary filed 257 `friction.error_screen` rows from one session and outranked `app.open`. Pick per call site: a handler is `track()`, a scrubbed control is `trackSettled()`, a mount that can loop is `trackOnce()`. After instrumenting, read the Usage ranking: anything above `app.open` is firing more than once per user action. (L-024)
- Never leave a column-sizing declaration (`width: 1%`, `max-width: 0`) on a bare class a `<span>` can pick up. A flex item's automatic minimum size is its SPECIFIED size, so `width: 1%` becomes the floor and `nowrap` text paints straight over its neighbour, invisible to typecheck, lint, and component tests. Scope the width half to `td`/`th`, keep the typography half (`tabular-nums`, alignment, `white-space`) global, give the long text column `.a-cell-fill` + `truncate` (never `line-clamp-1`, which has nothing to wrap on and collapses to the first word), and scope rail-only `::before` markers to `.a-rail` so the mobile strip does not paint them over the previous tab. Layout bugs need layout to verify: `bigdev/e2e/admin-ui-audit.py` renders every `/admin` page at four widths against mocked fixtures and reports overlap/clipping from the DOM (intersect each rect with its clipping ancestors first, or a scrolled-out row reads as a false positive). (L-025)
- A staleness threshold is `max(k * cadence, absolute floor)`, never a bare cadence multiple: `lastRunAt` stamps at tick FINISH, so one slow chain read trips a 500ms-2s worker, which is why `worker_staleness` went critical and `/health/ready` reported `degraded` against a healthy backend. Keep the rule in ONE exported function (`isWorkerStale()` in `worker-registry.ts`, 3x cadence AND 60s) shared by the detector, `/health/ready`, and the dashboard so health cannot fork into two answers, and treat a monitor that fires on its first run against a believed-healthy system as a threshold bug. (L-026)
- A wrong dashboard number is indistinguishable from a healthy product, so pin the unit at the query and separate NO DATA from ZERO: filter money aggregates by `coinType` (a bare `kind: 'send'` mixes SUI and DUSDC into one figure), say "one sample" instead of drawing a single-point SVG path that reads as zero, and render a failed health read as amber "unknown" rather than as no banner. All three are invisible to typecheck and to fixture tests, because a fixture is always well formed; the check is opening the page against real rows and asking whether every number states its unit and denominator. (L-027)
- Drive every Sui error matcher with a message captured VERBATIM off chain, never a hand-written string in the shape you remember, and parse BOTH abort shapes: the rust debug form (`MoveAbort(MoveLocation { name: <mod>, function_name: <fn> }, <code>)`) and the gRPC json form (`{"$kind":"MoveAbort",...,"abortCode":"1","location":{"module":...,"functionName":...}}`), falling back to the quoted `'<addr>::module::function'` path, which is prose and survives a wire-format change. Parsing only the first collapsed every real abort into one `MoveAbort unknown::unknown 0` group and stopped `chain.settle_already_redeemed` (gated on code `1`) from ever firing. A capture surface with zero rows is UNVERIFIED, not quiet: exercise it once through the real path, and wait for the flush, since capture is fire-and-forget. (L-028)
- A detector whose reading is a DURATION must derive it from something that outlives the process: `live_markets` counted minutes-at-zero in a module-level variable, so a redeploy or `bun --watch` reload rewound the clock and the banner read green through a ten-hour outage with the product unplayable. Resume from the persisted ops snapshot (already written every tick) on the first tick after a restart. Ask of any monitor state held in module scope what it reports after a restart mid-incident; if the answer is "green", persist it. (L-029)
- An id that still resolves is not a live protocol. Mysten redeployed Predict as `predict-testnet-7-29` (`0xfe742239…`) and killed the old `6-24` deployment's oracle publisher, so for ~62h every mint aborted on `pricing::live_inputs` code 4 `EBlockScholesPriceStale` (10s freshness window) while the existence-only boot guard stayed green and settlement kept working off the exact-timestamp lane. Liveness must exercise what breaks: `verifyLivePricing()` simulates `load_live_pricer` against a live market at boot. When a dependency you do not operate goes quiet, check for a REDEPLOY first (`gh api repos/MystenLabs/deepbookv3/branches` -> newest `predict-testnet-<date>` -> `packages/predict/deployment/deployment.testnet.json`). A redeploy is never an id swap: 7-29 merged bsSpot+bsForward into one `BlockScholesValueStore` (3 feeds, and the BS spot has no readable latest lane, read Pyth Lazer), added `max_cost` to `mint_exact_amount` and `min_probability`/`min_proceeds` to `redeem_live`, split the keeper path out to `redeem_settled_permissionless`, dropped the oracle args from `rebalance_expiry_cash`, changed `active_expiry_markets` entries to `{expiry_market_id, expiry_ms}`, and shipped a NEW `AccountRegistry` that re-derives every wrapper address, stranding chips behind each cached `User.predictWrapperId` (`scripts/migrate-predict-wrappers.ts` sweeps and clears). (L-031)
- A hook passed through one library into another is unproven until you watch it run. `SuiGrpcClient`'s constructor forwards only `baseUrl` + `fetchInit` into `GrpcWebFetchTransport`, so a `fetch` option typechecks (the options type is a union carrying the transport's own `GrpcWebOptions`) and is then silently dropped: build the transport yourself and pass `transport`, with the SDK version in a comment. Prove it with a config that can only work if the hook fires (dead primary + healthy fallback, assert a real chain read lands). Retry safety is per-method: a 429 was rejected before execution so re-issuing is always safe, but a connection error against `TransactionExecutionService` is ambiguous and must never be retried. The public fullnode allows ~100 req/30s per IP and every app on the host shares it, so `SUI_FULLNODE_URL` takes a comma-separated list, primary first. (L-032)
- Judge a game concept against the stated bar (surprise, no dead time, a spatial visual that IS the bet, escalation) BEFORE writing code, and append every rejection to `docs/games-ideation/README.md` §9 with its reason and a forward rule, or it gets re-pitched next round. Traps that killed six of ten round-1 concepts: correlated legs are not multiplay (if leg 3 hitting means legs 1-2 necessarily hit, N premiums buy ONE graded bet, which is what killed CLIMB); a game must be fun at N=1 (concurrency may make a game better, never make it exist); never put the hook downstream of unbuilt infrastructure; a payoff needing house money is blocked until the rake is on; a payoff that lands after the session ends leaves the console idle by construction; and every round needs at least one game where the machine DEALS, since round 1's four survivors were all pure-decision games. (L-033)
- Cost a play at the PRODUCT floor, not the protocol floor: `MIN_STAKE` is **$1.50** (`main-config.ts:238`), so a two-leg play is $3.00 and a four-leg play is $6.00. And assume **1x leverage**: `admittedMaxLeverage` (`services/games-real.ts`) returns 1x whenever the market is inside `no_leverage_window_ms`, which is ONE HOUR on testnet, so every 1m market caps at 1x and asking for more is a guaranteed `ELeverageAboveAdmissionCap` abort. Design payoffs on strike distance alone, count the mints, and state a play's true floor cost on screen before the player commits. Narrows L-009: the continuous leverage is real, just not at the cadence anything ships on. (L-034)
- Before calling a measurement device-only, find how the original number was produced and reproduce THAT instrument: the 514MB idle-tab figure came off an iOS Simulator reading host-side `com.apple.WebKit.WebContent` RSS (`bigdev/e2e/ios-idle-memory.py`, `simctl erase` per data point), so no in-page heap API is needed and no handset is either. Quote every cross-runtime figure as a delta over a control measured in the same session (a blank page reads 229MB on iOS 26.5 vs 169MB on 26.4, so absolute MB compare to nothing), rebuild the known-bad config as the upper control to prove the instrument still catches the regression, measure production builds not the dev server (~130MB high), and confirm every rendered reading with a screenshot: the install interstitial, the landing door, and a dev build all produced plausible-looking numbers for the wrong page. (L-030)
- If a rhythm is part of a game's design, DRAW it explicitly and draw it BEFORE the chain read (`rushDealChance(appetite)`, clamped, decided in the route), so a quiet beat is a decision rather than a solver's failure rate and costs no fullnode quota. RUSH's stream thinned backwards (8/8 dealt at 8x, 1/8 at 1.5x) because "quiet" was really "the solver missed the ask". Correct a solver in the space the quantity moves in: a multiple runs as `1/p`, so scale the PROBABILITY by the ratio it missed by (`p *= multiple / target`) and one step lands whatever the surface is doing; the chain's entry probability runs ~1.4x the one our sigma models. Measure any dealt stream read-only before shipping it, `quote_mint` devInspect needs no wrapper and no chips (`scripts/verify-rush-deals.ts`, the `verify-pin-ladder.ts` idiom). (L-035)
- **No apostrophes, or any lone quote character, in comments inside `EVENT_NAMES`** in `backend/src/config/analytics-catalog.ts` or `web/src/lib/track.ts`. `web/scripts/check-admin.ts` reads the array with `/['"]([^'"]+)['"]/g` over the raw source, so one apostrophe in a comment opens a quote and swallows every name after it. Write "the game is built for", never "the game's". If `check:admin` reports drift across dozens of names at once, suspect the parser, not the twins. (L-036)
- **Multi-leg is a first-class shape on real Predict, not a special case.** `mint_exact_amount` takes the `Pricer` by REFERENCE (`expiry_market.move:479`), so one `load_live_pricer` serves every leg in a PTB and each mint only needs its own fresh `Auth`: one `deposit_funds`, N mints, one `withdraw_funds`, wrapper shared last (`buildMintPlay` `legs[]`, `buildRedeemLivePlay` `orders[]`, `parseMints`/`parseRedeems` one result per leg, deduped by order id with the liquidation tombstone winning in either emission order). A play stores its legs comma-separated in the existing `Play.marketKey`, so no schema change, **but older backend code cannot settle one**: `BigInt("a,b")` throws and the settle worker retries forever rather than failing loudly, so roll the backend fully before anything multi-leg is reachable. When only one leg can pay, the play's multiple is the SMALLEST leg's payout over the WHOLE play's cost, never a sum. (L-037)
- When a rule applies to a SET (every lab game, every admin route, every money aggregate), drive the test off the same list production reads and iterate it (`it.each`), never a hand-picked representative. The lab's 404 gate was asserted on `pin` alone while four other games shipped behind it, and a name missing from `LAB` answers **400, not 404**, which still reads as a rejection. Assert both directions (denied for the ungated caller, reachable for the gated one) and assert the STATUS CODE, since 404-over-403 exists precisely because the two answers differ. Extends L-023: a test that has been run still proves only what it covers. (L-038)
- On the console the LABEL is the affordance, because `ActionSpec`/`KnobSpec` have no `disabled` and the hardware cannot dim. A control that cannot act during a live round says so in its own face (`format: () => (armed ? value : 'LOCKED')`); one that can act but would be destructive goes dead-neutral in colour while keeping its word (PRESS's `FOLD`). Never a silent early return, which reads as broken rather than locked, and never a cryptic glyph. PIN shipped with no guard at all, so its knob scrubbed a pin while the footer read the recorded one, showing two prices for one bet. If a dial is deliberately live all round, say why at the binding (RUSH's `APPETITE`). (L-039)
...
Current wave: **none.** `bigdev/CONT-TODO.md` holds the no-active-wave stub plus what wave 006 left open. Last shipped: **Wave 006, five new games + the games lab** (`bigdev/plans/cont/waves/WAVE-006.md`, spec `bigdev/plans/cont/04-GAMES-LAB.md`, designs `docs/games-ideation/`). Wave specs + archive: `bigdev/plans/cont/`. Lessons: `bigdev/claude/lessons.md`.
