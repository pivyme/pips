...
| `bigdev/plans/cont/03-ADMIN-DASHBOARD.md` | **In-house admin dashboard (error tracking + product analytics + ops health).** `User.specialRoles` + ADMIN gate, DB-backed settings in `AppConfig` (zero new env vars), Sui-aware error fingerprinting/grouping, the AI brief, the sealed `web/src/admin/` desktop dashboard, the event allowlist + `track()` contract, 12 ops detectors, retention guardrails. Read for anything analytics, error tracking, roles, or `/admin` |
